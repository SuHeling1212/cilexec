...
本文面向第一次接触编程的人，说明当前 FCL v0.0.2 中的“类”和“对象”是什么、怎样写，以及数据在复制和重启后会怎样。这里描述的是当前已实现的行为；完整函数清单见 [FCL 函数参考](fcl-function-reference.md)。
...
| CilExec 产品版本 | `0.0.2` |
| FCL 语言版本 | `fcl-0.0.2` |
| FCL 程序与 Continuation 格式 | `2` |
| 数据库迁移 | `V002`（Schema 最高版本 `2`） |
