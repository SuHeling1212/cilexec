 ...
 */
public class PrivilegedFunctionProvider extends BuiltinFunctionProvider {
    ...
    public Object call(String functionName, List<Object> args, FunctionContext context) {
        ...
        try {
            switch (functionName) {
                ...
                // 通过 Java 反射调用 JDK 或当前 Cilexec 二进制中的静态方法
                ...
                case "invoke": {
                    ...
                    if (!isBinaryClass(clazz)) {
                        throw new SecurityException("Reflection target is outside the Cilexec binary: "
                                + className);
                    }
    ...
    private static boolean isBinaryClass(Class<?> target) {
        // Java platform classes are part of the runtime required by the single binary.
        if (target.getClassLoader() == null) return true;
        CodeSource builtinSource = PrivilegedFunctionProvider.class
                .getProtectionDomain().getCodeSource();
        CodeSource targetSource = target.getProtectionDomain().getCodeSource();
        return builtinSource != null && targetSource != null
                && Objects.equals(builtinSource.getLocation(), targetSource.getLocation());
    }
