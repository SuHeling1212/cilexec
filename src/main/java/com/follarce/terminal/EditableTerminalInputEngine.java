...
/** Stateful ANSI-aware line editor used by raw local and remote terminal inputs. */
class EditableTerminalInputEngine implements TerminalInput {
    ...
    private final TerminalInput.KeyModeTransport keyModeTransport;
    private final List<String> history = new ArrayList<>();
    private final StringBuilder pasteBuffer = new StringBuilder();
    private boolean inPaste;
    private boolean rejectedPaste;
    private TerminalInput.RawMode keyMode;
    private boolean remoteKeyMode;
    private boolean kittyProtocolActive;

    EditableTerminalInputEngine(InputStream stream, Console console) {
        this(stream, console, () -> TerminalDimensions.current().width());
    }

    EditableTerminalInputEngine(InputStream stream, Console console, IntSupplier terminalWidth) {
        ...
        this.keyModeTransport = source instanceof TerminalInput.KeyModeTransport transport
                ? transport : null;
    ...
    public String readLine(PrintWriter output, String prompt, boolean remember)
        ...
        TerminalInput.RawMode mode;
        try {
            mode = TerminalInput.RawMode.enable();
    ...
    private String decodeTextBatch(int first) throws IOException {
        ...
        return "{\"kind\":\"paste\",\"text\":\"" + TerminalInput.jsonEscape(text.toString()) + "\"}";
    }
    ...
    private String finishBracketedPaste() {
        ...
        if (rejectedPaste) {
            ...
            return "{\"kind\":\"paste_rejected\",\"limit\":"
                    + TerminalInput.MAX_BRACKETED_PASTE_CHARACTERS + "}";
        }
        String text = pasteBuffer.toString().replace("\r\n", "\n");
        pasteBuffer.setLength(0);
        return "{\"kind\":\"paste\",\"text\":\"" + TerminalInput.jsonEscape(text) + "\"}";
    }
    ...
    private void appendPaste(String value) {
        ...
        if (value.length() > TerminalInput.MAX_BRACKETED_PASTE_CHARACTERS - pasteBuffer.length()) {
    ...
    private String decodeTilde(int number) {
        return switch (number) {
            ...
            default -> "{\"kind\":\"raw\",\"sequence\":\""
                    + TerminalInput.jsonEscape("ESC[" + number + "~") + "\"}";
        };
    }
    ...
    private static String keyEvent(String key, boolean shift, boolean ctrl, boolean alt,
                                   String text) {
        return "{\"kind\":\"key\",\"key\":\"" + TerminalInput.jsonEscape(key)
                + "\",\"shift\":" + shift + ",\"ctrl\":" + ctrl
                + ",\"alt\":" + alt + ",\"text\":\"" + TerminalInput.jsonEscape(text) + "\"}";
    }

    private static String rawEvent(String sequence) {
        return "{\"kind\":\"raw\",\"sequence\":\"" + TerminalInput.jsonEscape(sequence) + "\"}";
    }
    ...
    public void finishKeyMode() throws IOException {
        ...
        TerminalInput.RawMode current = keyMode;
        keyMode = null;
        current.close();
    }
    ...
    private void beginKeyMode() throws IOException {
        if (console != null) {
            if (keyMode == null) keyMode = TerminalInput.RawMode.enable();
            return;
        }
        if (!remoteKeyMode) {
            remoteKeyMode = true;
    ...
    private String decodeByteKey(int first) throws IOException {
        if (first < 0 || first < 128) return TerminalInput.decodeKey(first, stream::read);
        int length = first >= 0xF0 ? 4 : first >= 0xE0 ? 3 : 2;
        byte[] bytes = new byte[length];
        bytes[0] = (byte) first;
        for (int index = 1; index < length; index++) {
    ...
    private String readRawLine() throws IOException {
        ...
        while (true) {
            ...
            if (value.length() > TerminalInput.MAX_SUBMISSION_CHARACTERS) {
                throw new TerminalInput.SubmissionLimitExceeded("Terminal line exceeds 256 Ki characters");
            }
        }
    }
    ...
    private static void requireSubmissionLimit(StringBuilder value) throws IOException {
        if (value.length() > TerminalInput.MAX_SUBMISSION_CHARACTERS) {
            throw new TerminalInput.SubmissionLimitExceeded("Terminal submission exceeds 256 Ki characters");
        }
    }
