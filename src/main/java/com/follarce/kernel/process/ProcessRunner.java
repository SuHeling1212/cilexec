...
import com.follarce.kernel.exception.ExceptionContext;
import com.follarce.kernel.exception.ProcessException;
import com.follarce.kernel.exception.RecoverableException;
...
public class ProcessRunner {
    ...
    private boolean isChildFinished(int childPid, String expectedGeneration) {
        String path = com.follarce.kernel.vfs.PathUtil.findProcessFilePathByPid(childPid);
        if (path == null || !com.follarce.kernel.vfs.FileUtil.exists(path)) return true;
        if (expectedGeneration != null) {
            Object actualGeneration = com.follarce.kernel.util.JsonUtil.getField(path, "ProcessGeneration");
    ...
    private static boolean removeProcessFile(int targetPid, String expectedGeneration) {
        String path = com.follarce.kernel.vfs.PathUtil.findProcessFilePathByPid(targetPid);
        if (path == null || !com.follarce.kernel.vfs.FileUtil.exists(path)) return false;
        java.util.concurrent.locks.ReentrantLock lock = com.follarce.kernel.util.JsonUtil.lockFile(path);
        try {
            if (!com.follarce.kernel.vfs.FileUtil.exists(path)) return false;
            Map<String, Object> current = com.follarce.kernel.util.JsonUtil.parseToMapStrict(
                    com.follarce.kernel.vfs.FileUtil.read(path));
            if (!expectedGeneration.equals(current.get("ProcessGeneration"))) return false;
            com.follarce.kernel.vfs.FileUtil.removeFile(path);
        } finally {
            lock.unlock();
        }
    ...
    private static String readProcessGeneration(int targetPid) {
        String path = Constants.SYSTEM_PROCESS_PATH + targetPid + ".proc";
        if (!com.follarce.kernel.vfs.FileUtil.exists(path)) return null;
        try {
            Object generation = com.follarce.kernel.util.JsonUtil.getField(path, "ProcessGeneration");
            return generation instanceof String ? generation.toString() : null;
        } catch (Exception e) {
            return null;
        }
